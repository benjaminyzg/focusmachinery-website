    ...
    <html lang="en">
      ...
      <body className="min-h-screen bg-white text-slate-800 font-sans antialiased">
        <div className="max-w-5xl mx-auto px-6 py-12">
          ...
          <nav className="flex space-x-6 mb-10 border-b border-slate-100 text-sm font-semibold tracking-tight relative z-50">
            <NavLink to="/" className={({ isActive }) => `pb-3 border-b-2 transition-colors ${isActive ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-400 hover:text-slate-600'}`}>
              Home
            </NavLink>
            ...
            >
              <NavLink to="/products" className={({ isActive }) => `transition-colors ${isActive ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}>
              ...
              {openMenu === "products" && (
                <div className="absolute left-0 mt-3 w-56 bg-white border border-slate-100 rounded-md shadow-lg py-1 animate-fadeIn">
                  {subMenus.products.map((item) => (
                    <a key={item.name} href={item.href} className="block px-4 py-2 text-xs text-slate-600 hover:bg-slate-50 hover:text-indigo-600 transition">
                      {item.name}
                    </a>
                  ))}
                </div>
            ...
            <div 
              ...
              onMouseLeave={() => setOpenMenu(null)}
            >
              <NavLink to="/services" className={({ isActive }) => `transition-colors ${isActive ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}>
            ...
            >
              <div className="relative pb-0" onMouseEnter={() => setOpenMenu("resources")}>
                <NavLink to="/resources" className={({ isActive }) => `transition-colors ${isActive ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}>
            ...
            >
              <div className="relative pb-0" onMouseEnter={() => setOpenMenu("about")}>
                <NavLink to="/about" className={({ isActive }) => `transition-colors ${isActive ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}>
            ...
            <div 
              ...
              onMouseLeave={() => setOpenMenu(null)}
            >
              <NavLink to="/contact" className={({ isActive }) => `transition-colors ${isActive ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-600'}`}>
